...
async function getAllOffers(req, res, next){
    try{
        ...
        console.error('Не удалось получить все предложений',error); e
...
export async function getFullOffer(req, res, next){
    try{
        ...
        //проверить, что предложение существует
        if (!offer){
            return next(ApiError.badRequest('Предложение не найден :('))
        }
        //преобразовать предложение в формат клиента
...
export async function createOffer(req, res, next){
    try{
        const{
            ...
            guests, price, features, commentsCount, latitude, 
            longitude, userId
        } = req.body;

        if(!req.files?.previewImage || req.files.previewImage.length === 0) {
            return next(ApiError.badRequest('Первью изображение обязательно для загрузки'));
        ...
        let processedPhotos = [];
        if (req.files?.photos) {
            processedPhotos = req.files.photos.map(file => 
...
const getFavoriteOffers = async (req, res, next) => {
    try{
        ...
        next(ApiError.internal('Ошибка получения избранных предложений: '+ error.message));
    }
}
